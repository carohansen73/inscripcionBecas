    ...
    <table id="table" class="table table-striped table-bordered nowrap" style="width:100%">
        ...
        @foreach($inscriptions as $inscription)
            <tr>
                ...
                <td>
                    <div class='btn-group'>
                    {!! Form::open(['route' => ['inscripciones.destroy', $inscription->id], 'method' => 'delete']) !!}
                    
                        <a href="{{ route('inscripciones.show', [$inscription->id]) }}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-eye-open"></i></a>
                        {{-- se comentan botones editar y borrar fuera de la fecha de inscripcion--}}
                       <a href="{{ route('inscripciones.edit', [$inscription->id]) }}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-edit"></i></a>
                        {!! Form::button('<i class="glyphicon glyphicon-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-danger btn-xs', 'onclick' => "return confirm('Desea eliminar de forma permanente la inscripción?')"]) !!}
